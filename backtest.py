...
def price_trade(client, symbol, transcript_dt, interval=INTERVAL):
    """
    Returns (entry_date, entry_price, gross_returns_dict) or None.
    Entry = open of first trading session whose 09:30 falls strictly after the
    ...
    trading_days = sorted({b["dt"].date() for b in bars
                           if b["dt"].date() > dec.date() or
                           (b["dt"].date() == dec.date() and dec.time() < RTH_OPEN)})
    # entry day = first trading day strictly after the decision moment
    entry_day = next((d for d in trading_days if d > dec.date()), None)
